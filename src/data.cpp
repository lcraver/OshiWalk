...
int data_read_gif_index(void) {
    nvs_handle_t h;
    if (nvs_open("prefs", NVS_READONLY, &h) != ESP_OK) return -1;
    int8_t idx = -1;
    nvs_get_i8(h, "gif_idx", &idx);
    nvs_close(h);
    return (int)idx;
}
...
bool data_write_gif_index(int index) {
    nvs_handle_t h;
    if (nvs_open("prefs", NVS_READWRITE, &h) != ESP_OK) return false;
    esp_err_t err = nvs_set_i8(h, "gif_idx", (int8_t)index);
    nvs_commit(h);
    nvs_close(h);
    return err == ESP_OK;
}

static void avatar_defaults(avatar_config_t *cfg) {
    memset(cfg, 0, sizeof(*cfg));
}

bool data_read_avatar(avatar_config_t *cfg) {
    nvs_handle_t h;
    if (nvs_open("avatar", NVS_READONLY, &h) != ESP_OK) { avatar_defaults(cfg); return false; }
    size_t sz = sizeof(avatar_config_t);
    esp_err_t err = nvs_get_blob(h, "cfg4", cfg, &sz);
    nvs_close(h);
    if (err != ESP_OK || sz != sizeof(avatar_config_t)) { avatar_defaults(cfg); return false; }
    return true;
}

bool data_write_avatar(const avatar_config_t *cfg) {
    nvs_handle_t h;
    if (nvs_open("avatar", NVS_READWRITE, &h) != ESP_OK) return false;
    esp_err_t err = nvs_set_blob(h, "cfg4", cfg, sizeof(avatar_config_t));
    nvs_commit(h);
    nvs_close(h);
    return err == ESP_OK;
}
